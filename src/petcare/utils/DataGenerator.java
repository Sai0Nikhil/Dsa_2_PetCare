...
public class DataGenerator {
    ...
    private static final String[] DIAG    = {"Fever","Fracture","SkinAllergy","Dental","Vaccination","Checkup"};
    ...
    public static List<Pet> generatePets(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            pets.add(new Pet(100 + i, pick(NAMES, r), pick(SPECIES, r), pick(BREEDS, r),
                    1 + r.nextInt(15), pick(OWNERS, r)));
        }
        return pets;
    }
    ...
    public static List<Treatment> generateTreatments(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            ts.add(new Treatment(1000 + i, 100 + 1 + r.nextInt(10),
                    pick(DIAG, r), pick(MEDS, r), 200 + r.nextInt(1800), 1 + r.nextInt(30)));
        }
        return ts;
    }
    ...
    public static List<Appointment> generateAppointments(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            as.add(new Appointment(9000 + i, 100 + 1 + r.nextInt(10), start, end, pick(DIAG, r)));
        }
        return as;
    }
    ...
    public static List<Vaccination> generateVaccinations(int n) {
        ...
        for (int i = 1; i <= n; i++) {
            vs.add(new Vaccination(5000 + i, 100 + 1 + r.nextInt(10),
                    pick(VACC, r), 1 + r.nextInt(60)));
        }
        return vs;
    }
